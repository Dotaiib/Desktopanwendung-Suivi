...
namespace WFApp_Suivi
{
    public partial class Form5 : Form
    {
        ...
        private void Afficher()
        {
            ...
            string req = "select * from Receveur order by Date_Time desc";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Text == "")
            {
                MessageBox.Show("Nom & Prénom est vide !! Entrez une Valeur!");
            }
            else
            {
                cn.Open();
                SqlCommand cmd = new SqlCommand("insert into Receveur values('" + textBox1.Text + "',getdate())", cn);
                cmd.ExecuteNonQuery();
                cn.Close();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                cn.Open();
                SqlCommand cmd = new SqlCommand("select * from Receveur where Code_Receveur Like '" + textBox1.Text + "'", cn);
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    dataGridView1.Rows.Clear();
